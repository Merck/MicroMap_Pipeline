...
zipfile1<-paste(Name,"_",Target,"Target_",Metric,"_",Normalization,"_Datafiles_wPep",sep="")
zip(zipfile=zipfile1,files=files1)
file.rename(from=paste(datadir,zipfile1,".zip",sep=""),to=paste(outdir,zipfile1,".zip",sep=""))

setwd(imgdir)
files2<-dir(imgdir)
files2<-files2[grepl("AltSize.pdf",files2)|grepl("AltSize.tiff",files2)|grepl("AltSize.jpg",files2)|grepl("Peptide.pdf",files2)|grepl("Peptide.jpg",files2)|grepl("Peptide.tiff",files2)]
zipfile2<-paste0(Name,"_",Metric,"_",Normalization,"_Images_wPep")
